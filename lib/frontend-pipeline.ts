...
export class FrontendPipeline extends cp.Pipeline {
  constructor(scope: Construct, id: string, props: FrontendPipelineProps) {
    ...
    if (repositoryParts.length !== 2) {
      Annotations.of(this).addError('sourceRepository must be in the format <organization>/<repository> (e.g. sbam-cloud/notes-frontend)');
    }
    ...
    this.stage('Build').addAction(new cpActions.CodeBuildAction({
      actionName: 'Build',
    ...
    this.stage('Deploy').addAction(new cpActions.S3DeployAction({
      actionName: 'Deploy',
      bucket: props.destinationBucket,
      input: buildArtifact,
      cacheControl: [
        cpActions.CacheControl.maxAge(Duration.days(365)),
      ],
    }));
    this.stage('Cleanup').addAction(new InvalidateCloudfrontCodepipelineAction(this, {
